...
import { createTRPCRouter, protactedProcedure } from "@/trpc/init";
import z from "zod";
...
export const meetingsRouter = createTRPCRouter({
  generateToken: protactedProcedure.mutation(async ({ ctx }) => {
    await streamVideo.upsertUsers([
      {
        ...
        image:
          ctx.auth.user.image ??
          genrateAvatarUri({ seed: ctx.auth.user.name, varient: "initials" }),
      },
    ]);
